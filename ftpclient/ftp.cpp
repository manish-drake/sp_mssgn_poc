...
void ftp_t::get_file(const char *file_name, const std::string &destination)
{
  ...
  if ("213" == str_code)
  {
    ...
    char *stopstring;
    size_file = std::strtoul(str_size.c_str(), &stopstring, 10);
...
void ftp_t::send_file(FILE *fp, int sockfd)
{
  int n;
  ...
  {
    size_t szSent = send(sockfd, data, sizeof(data), 0);
    if (szSent == -1)
    {
...
bool ftp_t::senddata(int sock, void *buf, int buflen)
{
    char *pbuf = (char *) buf;
...
bool ftp_t::sendfile(int sock, FILE *f)
{
    ...
    if (filesize > 0)
    {
        ...
        do
        {
            size_t num = std::min(filesize, sizeof(buffer));
